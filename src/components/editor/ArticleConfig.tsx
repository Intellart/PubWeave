...
function ArticleConfig({
  ...
  const category = find(
    categories,
    (c) => c.category_name === get(article, "category", null)
  );
  ...
  const dif = differenceInSeconds(currentTime, lastSaved);
  ...
  const lastSavedString = () => {
    if (dif < 10) {
      ...
      return `${floor(dif / 60)} minute${floor(dif / 60) > 1 ? "s" : ""} ago`;
    } else if (dif < 86400) {
  ...
  const tagOptions = map(
    pickBy(allTags, (tag: Tag) => tag.category_id === category?.id),
    (tag: Tag) => ({
      label: tag.tag,
      value: tag.id,
    })
  ...
  const handleSaveTags = () => {
    ...
    const tagsToRemove: Tags = omitBy(articleTags, (tag) =>
      get(tags, tag.id, null)
    );
    const tagsToAdd: Tags = omitBy(tags, (tag) =>
      get(articleTags, tag.id, null)
    );
  ...
  const onNewTagClick = (values: BasicOption[]) => {
    setTags(
      pickBy(allTags, (tag) =>
        find(values, (v: BasicOption) => v.value === tag.id)
      )
    );
  };
  ...
  return (
    <>
      <div
        ...
        className={classNames("article-config-backdrop", {
          hidden: !articleSettingsExpanded,
        })}
      />
      <div
        style={{
        ...
        className={classNames("article-config-small", {
          hidden: articleSettingsExpanded,
        })}
      >
      ...
      <div
        className={classNames("article-config-large", {
          hidden: !articleSettingsExpanded,
        })}
      >
        <div className="article-config-item">
        ...
        <div className="article-config-item">
          ...
          <div className="article-config-item-tags">
            ...
            id="standard-basic" */}
            <div className="article-config-item-tags-buttons">
              {tagsChanged && (
                <div
