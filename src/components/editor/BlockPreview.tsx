...
import { find, findIndex, forEach, get, includes, isEqual } from "lodash";
...
function Editor({
  ...
  const labelCriticalSections = () => {
    forEach(
      ...
      (div, divIndex) => {
        if (div) {
          const isCritical = includes(
            criticalSectionIds,
            get(blocks[divIndex], "id")
          );
  ...
  const checkIfCriticalSection = (newBlocks: any) => {
    ...
    forEach(newBlocks, (block) => {
      if (includes(criticalSectionIds, get(block, "id"))) {
        const oldBlockInd = findIndex(
          blocks,
          (o) => get(o, "id") === get(block, "id")
        );
        const newBlockInd = findIndex(
          newBlocks,
          (o) => get(o, "id") === get(block, "id")
        );
    ...
    editor.current?.save().then((newArticleContent: ArticleContent) => {
      ...
      if (onChange) {
        ...
        if (versioningBlockId.current !== null) {
          ...
          const oldBlock = find(
            blocks,
            (o) => get(o, "id") === versioningBlockId.current
          );
          const newBlock = find(
            newArticleContent.blocks,
            (o) => get(o, "id") === versioningBlockId.current
          );
