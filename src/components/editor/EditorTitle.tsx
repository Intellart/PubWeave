  ...
  useEffect(() => {
    if (titleRef.current) {
      ...
      if (articleTitle.length > maxPx && articleTitle.length < maxChars) {
        const fontSize =
          maxPx +
          ((minPx - maxPx) / (maxChars - minChars)) *
            (articleTitle.length - minChars);
        titleRef.current.style.fontSize = `${fontSize}px`;
        titleRef.current.style.width = `${
          articleTitle.length * fontSize * 0.55 + 60
        }px`;
  ...
  return (
    <>
      ...
      >
        {!titleFocus &&
          (!inReview ? (
            <div className="editor-publish-button-back" />
          ) : (
            <Link
              onClick={(e) => {
                e.stopPropagation();
      ...
      <hr
        className={classNames("editor-title-hr", {
          focus: titleFocus,
          empty: !articleTitle || articleTitle === "New article",
        })}
      />
