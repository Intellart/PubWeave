...
import { get, isEqual, map, sortBy, uniqBy } from "lodash";
import {
  Autocomplete,
  Button,
  TextField,
...
function Navbar(props: Props) {
  ...
  const renderLoginButton = () => {
    if (!isMobile && (props.isAuthorized || props.isAdmin)) {
      ...
      return (
        <Link onClick={handleClick} className="login-button" to="/login">
          Login
        </Link>
      );
    }
  ...
  const renderSearch = () => (
    <div className="search-wrapper">
      ...
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            label={
              searchParams.type === searchType.AUTHOR
                ? "Search Authors"
                : "Search Articles"
            }
          />
      ...
      >
        <ToggleButton className="search-type-button" value={searchType.AUTHOR}>
          <FontAwesomeIcon className="search-type-button-icon" icon={faUser} />
        </ToggleButton>
        <ToggleButton className="search-type-button" value={searchType.ARTICLE}>
          <FontAwesomeIcon
            className="search-type-button-icon"
            icon={faScroll}
          />
        </ToggleButton>
      </ToggleButtonGroup>
    ...
    <>
      <nav className="navbar">
        ...
        <div className="navigation">
          ...
          <NavLink onClick={handleClick} to="/blogs">
            Blogs{" "}
          </NavLink>
          {props.isAdmin && (
      ...
      {isMobile && (
        <>
          <div
            className={classNames("navbar-mobile-menu-overlay", {
          ...
          <div
            ref={ref}
            className={classNames("navbar-mobile-menu", {
              "mobile-menu-open": mobileMenuOpen,
            })}
          >
