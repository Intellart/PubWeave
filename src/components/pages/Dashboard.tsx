...
function Dashboard() {
  ...
  const publishArticle = (articleId: number, status: string) =>
    dispatch(articleActions.publishArticle(articleId, status));
  const updateArticle = (id: number, payload: any) =>
    dispatch(articleActions.updateArticle(id, payload));
  ...
  useEffect(() => {
    if (!articles) {
      ...
      setRows(
        map(
          filter(articles, (a) => a.status !== "draft"),
          (article) => ({
            id: article.id,
            title: article.title,
  ...
  return (
    <main className="dashboard-wrapper">
      ...
      <Alert sx={{ width: "90%" }} severity="info">
        ...
        - Publish an article
        <br />
        - Edit the article <FontAwesomeIcon icon={faEdit} />
        <br />
        - Change the category of an article
        <br />
        - Change the status of an article
        <br />
        - Star an article <FontAwesomeIcon icon={faStar} />
        <br />
      </Alert>
      <Alert sx={{ width: "90%" }}>
        <AlertTitle>Legend</AlertTitle>
        Draft &gt; Requested (by author)
        <br />
        Requested &gt; Published (by admin)
        <br />
