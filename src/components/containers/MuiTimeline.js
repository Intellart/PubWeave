...
function MuiTimeline(props: Props): Node {
  const numOfSections = props.sections.length;

  const [activeSection, setActiveSection] = React.useState(props.activeSection);
    ...
    <Timeline position="alternate" className='timeline'>
      {map(props.sections, (section, index) => (
        <TimelineItem key={index}>
          <TimelineSeparator className={classNames('timeline-seperator', `timeline-seperator-${props.activeSection}`)}>
            <TimelineDot
              className={classNames('timeline-dot', `timeline-dot-${props.activeSection}`)}
