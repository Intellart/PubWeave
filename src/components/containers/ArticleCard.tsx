...
function ArticleCard(props: Props) {
  ...
  const chipParams = (imageChip: boolean = false) => {
    ...
    const numOfReviewers = size(
      filter(
        get(props.article, "reviewers", []),
        (reviewer) => !!reviewer.review_content
      )
    );
  ...
  return (
    <div className="article-card">
      <div
        ...
        className={classNames("article-card-img-wrapper", {
          "article-card-img-wrapper-published":
      ...
      >
        ...
        />
        {props.showPublishedChip && (isPublished || noImage) && (
          <Chip className="article-card-img-chip" {...chipParams(true)} />
        )}
      </div>
      <div className="article-right-side-content">
        <div className="article-card-side-content-upper-wrapper">
          <div
            onClick={handleArticleClick}
            className="article-card-side-content-title-wrapper"
          >
            <h4>{props.article.category || "Category"}</h4>
            <h2>{props.article.title}</h2>
            <p className="author">
              By {props.article.author.full_name || "Authors Name"}
            </p>
          </div>
          <div className="article-card-side-content-status-wrapper">
            ...
            <ArticleTypeModal
              enabled={get(
                currentPermissions,
                permissions.SWITCH_ARTICLE_TYPE,
                false
              )}
              type={workType}
        ...
        <p className="article-card-description">
          {description.substring(0, isMobile ? 45 : 200)}...
        </p>
        <div className="article-card-lower">
          <div className="date-social">
            <p>
              Updated {new Date(props.article.updated_at).toLocaleDateString()}
            </p>
            <div className="article-icons-share-heart">
              ...
              {get(currentPermissions, permissions.ARTICLE_SETTINGS, false) && (
                <Link
                  to={`/my-work/${props.article.id}/settings`}
              ...
              {get(currentPermissions, permissions.DELETE_ARTICLE, false) && (
                <a onClick={(e) => handleDeleteArticle(e)}>
