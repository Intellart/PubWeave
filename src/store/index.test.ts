...
describe("chainActionsMiddleware", () => {
  ...
  test("is not included in store middleware if actionChain object is not passed", () => {
    ...
    configureStore({});
    const applyMiddlewareArgumentsSize =
      mockedApplyMiddleware.mock.calls[0].length;
  ...
  test("dispatches action (when string) set in actionChain object", () => {
    chainActionsMiddleware({ ACTION_ONE: "ACTION_TO_FOLLOW_ACTION_ONE" })({
      dispatch: mockedDispatch,
    })(() => {})({ type: "ACTION_ONE" });
    expect(mockedDispatch).toHaveBeenCalledWith({
      type: "ACTION_TO_FOLLOW_ACTION_ONE",
    });
  });
  ...
  test("dispatches actions (when array) set in actionChain object", () => {
    chainActionsMiddleware({
      ACTION_ONE: [
        ...
        () => ({
          type: "SECOND_ACTION_TO_FOLLOW_ACTION_ONE",
          payload: "Dummy",
        }),
    ...
    expect(mockedDispatch.mock.calls[0][0]).toEqual({
      type: "FIRST_ACTION_TO_FOLLOW_ACTION_ONE",
      payload: "Dummy",
    });
    ...
    expect(mockedDispatch.mock.calls[2][0]).toEqual({
      type: "THIRD_ACTION_TO_FOLLOW_ACTION_ONE",
    });
  });
});
